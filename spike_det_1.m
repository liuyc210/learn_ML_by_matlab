...
for sampleIndex = 1: sampleNum
    ...
    if (targets(sampleIndex) == 1)
        pos = unidrnd(8,1,1);
        inputs(:,sampleIndex) = inputs(:,sampleIndex) + [zeros(1,pos), ones(1,1), zeros(1,sampleDimen-pos-1)]';
    end
...
net.layers{1}.size = 15;  %increasing size is very important
net.biasConnect = [1;1];
...
for sampleIndex = 1: sampleNum
    ...
    if (targets(sampleIndex) == 1)
        pos = unidrnd(8,1,1);
        inputs(:,sampleIndex) = inputs(:,sampleIndex) + [zeros(1,pos), ones(1,1)*1, zeros(1,sampleDimen-pos-1)]';
    end
