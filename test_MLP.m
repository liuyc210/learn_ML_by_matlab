...
elseif (caseID == 6)           %  difficult, may needs more nIter
    freq = 1.2;                  % high freq need more hidden nodes, e.g. freq=3, nHidden = 8;
    x = linspace(-1,1,nSample/2);
    y1 = sin(freq*pi*x)*.5;   
    y2 = y1 + .5;
    inputs = [ [x,x]; [y1,y2] ];
...
%     L1 = L1 - lr*delta1(1:nHidden,:)*i_L1';
    ...
    delta2 = repmat(L3',1,nSample).*repmat(delta3, nH2+1,1).*i_L3.*(1-i_L3);
    delta1 = repmat(L2',1,nSample).*repmat(delta2, nHidden+1,1).*i_L2.*(1-i_L2);
    L3 = L3 - lr*delta3*i_L3' ;
    L2 = L2 - lr*delta2(1:nH2,    :)*i_L2';
    L1 = L1 - lr*delta1(1:nHidden,:)*i_L1';
...
if (nHidden == 3)
    ...
    grid_x = reshape(o_L1(1,:), ll, ll);
    grid_y = reshape(o_L1(2,:), ll, ll);
    grid_z = reshape(o_L1(3,:), ll, ll);
    surf(grid_x, grid_y, grid_z); title('2D grid in 3D space');
end
